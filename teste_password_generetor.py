...
class TestPasswordGenerator(unittest.TestCase):
    def test_make_one_special_char_multiple_ranges(self):
        regex = re.compile(r'^[ -\/:-@\[-`{-~]$')  # expressão regular
    ...
    def test_make_one_uppercase_letter_range_65_90(self):
        regex = re.compile(r'^[A-Z]$')  # expressão regular
    ...
    def test_make_one_number_range_48_57(self):
        regex = re.compile(r'^[0-9]$')  # expressão regular
    ...
    def test_make_password_no_lowers(self):
        regex = re.compile(r'^[^a-z]+$')
    ...
    def test_make_password_sequence_not_allowed(self):
        ...
        for i in range(4, 1000):
            with self.subTest(i=1):
                # se a condição de regex não estiver presente
