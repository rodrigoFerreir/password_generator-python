...
def make_password(length=16, chars=True, lower=True, upper=True, numbers=True):
    # a senha deve ter no minimo 4 caracteres não pode recerber numeros no length
    # pelo menos um parametro deve ser true
    assert isinstance(length, int), 'length, must be of type int'
    assert length >= 4, 'length must be at least 4 chars'
    assert chars or lower or upper or numbers, 'At list one param must be True'
